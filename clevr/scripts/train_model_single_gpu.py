...
parser.add_argument('--num_train_samples', default=2048, type=int)
...
parser.add_argument('--debug_every', default=120, type=float) 
...
def main(args):
  ...
  val_loader_kwargs = {
    ...
    'max_samples': args.num_val_samples, # 149991
...
def train_loop(args, train_loader, val_loader):
  ...
  if args.model_type in ['FiLM', 'EE', 'PG+EE']:
    ...
    ee_optimizer = optim_method(execution_engine.trainable_params(),
    ...
    for batch in train_loader:
      ...
      if args.model_type == 'FiLM':
        ...
        if args.train_execution_engine == 1:
          if args.grad_clip > 0:
            torch.nn.utils.clip_grad_norm(execution_engine.trainable_params(), args.grad_clip)
          ee_optimizer.step()
...
def check_accuracy(args, program_generator, execution_engine, baseline_model, loader):
  ...
  for batch in loader:
    ...
    if scores is not None:
      ...
      # this is for checking how can train_acc be low when loss -> 0.
      prob_value, preds = scores.data.cpu().max(1)
...
def check_grad_num_nans(model, model_name='model'):
    grads = [p.grad for p in model.trainable_params() if p.grad is not None]
