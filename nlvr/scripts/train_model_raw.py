...
parser.add_argument('--feature_dim', default='3,14,56')
...
parser.add_argument('--num_train_samples', default=80000, type=int)
...
def train_loop(args):
    ...
    f_vocab = open('data/nlvr_vocab_raw.pkl', 'rb')
    vocab, inv_vocab = pkl.load(f_vocab)
    args.vocab = vocab
    f_vocab.close()
    
    # load data
    train_image = h5py.File('../../../dataset/NLVR/h5/nlvr_train_raw.h5', 'r')['image']
    train_sent_int = h5py.File('../../../dataset/NLVR/h5/nlvr_train_raw.h5', 'r')['sent']
    train_labels = h5py.File('../../../dataset/NLVR/h5/nlvr_train_raw.h5', 'r')['labels']
    
    dev_image = h5py.File('../../../dataset/NLVR/h5/nlvr_dev_raw.h5', 'r')['image']
    dev_sent_int = h5py.File('../../../dataset/NLVR/h5/nlvr_dev_raw.h5', 'r')['sent']
    dev_labels = h5py.File('../../../dataset/NLVR/h5/nlvr_dev_raw.h5', 'r')['labels']
    
    test_image = h5py.File('../../../dataset/NLVR/h5/nlvr_test_raw.h5', 'r')['image']
    test_sent_int = h5py.File('../../../dataset/NLVR/h5/nlvr_test_raw.h5', 'r')['sent']
    test_labels = h5py.File('../../../dataset/NLVR/h5/nlvr_test_raw.h5', 'r')['labels']
    
    train_image, train_sent_int, train_labels = train_image[:args.num_train_samples], train_sent_int[:args.num_train_samples], train_labels[:args.num_train_samples]
    dev_image, dev_sent_int, dev_labels = dev_image[:args.num_val_samples], dev_sent_int[:args.num_val_samples], dev_labels[:args.num_val_samples]
    test_image, test_sent_int, test_labels = test_image[:args.num_test_samples], test_sent_int[:args.num_test_samples], test_labels[:args.num_test_samples]
    
    args.rnn_time_step = len(train_sent_int[0])
    print args.rnn_time_step        
    ...
    'train_accs': [], 'val_accs': [], 'val_accs_ts': [], 'test_accs':[], 'test_accs_ts':[],
    'best_val_acc': -1, 'model_t_val': 0, 'model_t_test': 0
    }
    t, epoch, reward_moving_average = 0, 0, 0

    set_mode('train', [program_generator, execution_engine, baseline_model])

    print('train_loader has %d samples' % len(train_sent_int))
    print('val_loader has %d samples' % len(dev_sent_int))
    ...
    while t < args.num_iterations:
        ...
        start_id = 0
        while start_id < len(train_sent_int):
            t += 1
            questions = torch.cuda.LongTensor(train_sent_int[start_id:(start_id + args.batch_size)])
            feats = torch.Tensor(train_image[start_id:(start_id + args.batch_size)])
            answers = torch.cuda.LongTensor(train_labels[start_id:(start_id + args.batch_size)])
            start_id += args.batch_size
            ...
            if t % args.checkpoint_every == 0:
                ...
                train_acc = check_accuracy(args, program_generator, execution_engine,
                                   baseline_model, train_sent_int, train_image, train_labels)
                if args.time == 1:
		...
		train_acc = -1
                ...
                val_acc = check_accuracy(args, program_generator, execution_engine,
                                 baseline_model, dev_sent_int, dev_image, dev_labels)
                if args.time == 1:
                ...
                test_acc = check_accuracy(args, program_generator, execution_engine,
                                 baseline_model, test_sent_int, test_image, test_labels)
                if args.time == 1:
                    test_pass_time = (time.time() - start)
                    test_pass_total_time += test_pass_time
                    print(colored('TEST PASS AVG TIME:   ' + str(test_pass_total_time / num_checkpoints), 'cyan'))
                    print(colored('Test Pass Time        : ' + str(test_pass_time), 'cyan'))
                ...
                stats['test_accs'].append(test_acc)
                stats['test_accs_ts'].append(t)
...
def check_accuracy(args, program_generator, execution_engine, baseline_model, sent_int, image, labels):
  set_mode('eval', [program_generator, execution_engine, baseline_model])
  num_correct, num_samples = 0, 0
  start_id = 0
  while start_id < len(sent_int):
    questions = torch.cuda.LongTensor(sent_int[start_id:(start_id + args.batch_size)])
    feats = torch.Tensor(image[start_id:(start_id + args.batch_size)])
    answers = torch.LongTensor(labels[start_id:(start_id + args.batch_size)])
    start_id += args.batch_size
