...
class FiLMedNet(nn.Module):
  def __init__(self, vocab, feature_dim=(1024, 14, 14),
    ...
    self.encoder_embed = nn.Embedding(encoder_vocab_size+1, wordvec_dim)
    ...
    self.extra_channel_freq = self.use_coords_freq # == 1
    ...
    module_H = feature_dim[1] // (stem_stride ** stem_num_layers)  # Rough calc: work for main cases  14/(1^1)
    module_W = feature_dim[2] // (stem_stride ** stem_num_layers)  # Rough calc: work for main cases  14/(1^1)
    self.coords = coord_map((module_H, module_W)) # size(2,module_H, module_W) expanded linspace.
    self.default_weight = Parameter(torch.ones(1, 1, self.module_dim).type(torch.cuda.FloatTensor), requires_grad=False) # to replace film, not used.
  ...
  def forward(self, questions, image_features, save_activations=False):
	...
	# self.module_outputs.append(block_output)
...
class LangMemBlock(nn.Module):
    def __init__(self, hidden_dim, with_batchnorm=True):
        ...
        self.hidden_dim = hidden_dim
        self.with_batchnorm = with_batchnorm
        # batch norm for rnn feature maps, not used
    ...
    def forward(self, state_seq, gs, ks, in_single, fn_num, print_flag):
        ...
        if self.att_mode == 'paramed': 
            states_mult = state_seq * ks.unsqueeze(1).expand(N,seq_len,dim)
