...
def build_cnn_proj(module_C, module_H, module_W,
                     proj_dim=None, downsample='maxpool2',
  ...
  if 'maxpool' in downsample or 'avgpool' in downsample:
    ...
    prev_dim = proj_dim * math.ceil(module_H / pool_size[0]) * math.ceil(module_W / pool_size[1])
...
def build_classifier(num_answers, fc_dims=[], proj_dim=None, with_batchnorm=True, dropout=0):
  layers = []
  prev_dim = int(proj_dim)
  for next_dim in fc_dims: # one layer 1024 dim
    next_dim = int(next_dim)
    print prev_dim, next_dim
